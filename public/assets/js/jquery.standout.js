...
;( function( $, window, undefined ) {
	...
	$.standout = function( options, element ) {
	...
	// the options
	$.standout.defaults = {
		// if we want to specify a selector that triggers the next() function. example: '#standout-nav-next'
		nextEl : '',
	...
	$.standout.prototype = {
		...
		_init : function( options ) {
			...
			// options
			this.options = $.extend( true, {}, $.standout.defaults, options );
		...
		_initEvents : function() {
			...
			if( this.options.nextEl !== '' ) {
				...
				$( this.options.nextEl ).on( 'click.standout', function() {
			...
			if( this.options.prevEl !== '' ) {
				...
				$( this.options.prevEl ).on( 'click.standout', function() {
			...
			this.$el.on( 'click.standout', 'li', function() {
		...
		_move2front : function( $item ) {
			...
			// if it's the one with higher z-index, just close the standout
			if( !this.closed ) {
	...
	$.fn.standout = function( options ) {
		...
		var instance = $.data( this, 'standout' );
		...
		if ( typeof options === 'string' ) {
			...
			this.each(function() {
				...
				if ( !instance ) {
					...
					logError( "cannot call methods on standout prior to initialization; " +
				...
				if ( !$.isFunction( instance[options] ) || options.charAt(0) === "_" ) {
					...
					logError( "no such method '" + options + "' for standout instance" );
					return;
			...
			this.each(function() {
				...
				if ( instance ) {
					...
					instance = $.data( this, 'standout', new $.standout( options, this ) );
